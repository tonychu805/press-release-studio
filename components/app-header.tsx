...
interface AppHeaderProps {
  ...
  onTabChange: (tab: "projects" | "workspace") => void
}

export function AppHeader({ onOpenSettings, activeTab, onTabChange }: AppHeaderProps) {
  const { projectName, setProjectName, status, resetProject, projectId } = useWorkflow()
  ...
  return (
    <header className="border-b border-border bg-card">
        ...
        <div className="flex items-center gap-2">
          {activeTab === "workspace" && projectId && (
            <>
              ...
              />
              <span className="hidden items-center gap-1.5 rounded-full border border-border bg-background px-2.5 py-1 text-xs font-medium text-muted-foreground lg:inline-flex">
                <span className="h-1.5 w-1.5 rounded-full bg-accent" />
                {STATUS_LABELS[status]}
              </span>
              <Button variant="ghost" size="icon" onClick={resetProject} title="Reset project" aria-label="Reset project">
      ...
      <div className="flex items-end gap-0 px-5">
        ...
        <button
          onClick={() => projectId && onTabChange("workspace")}
          disabled={!projectId}
          className={cn(
            "px-4 py-2 text-sm font-medium border-b-2 transition-colors",
            activeTab === "workspace"
